...
export default function AdminOfficeApprovalPage() {
  ...
  // Mandatory Signatory Verification Checkboxes (Section 3.2 & Step 3 Manual Specifications)
  ...
  const handleAdminApproval = async (e: React.FormEvent) => {
    ...
    if (action === 'APPROVE') {
      ...
      if (!adminProofFilePath) {
        setSystemError('AUDIT TRAIL FAILURE: Attached signature verification proof path URL is required for executive sign-off.');
        return;
      }
    }
          ...
          {action === 'APPROVE' && (
            <div className="bg-slate-50/80 border border-slate-200 rounded-xl p-4 space-y-4">
              ...
              <div className="border-t border-slate-200 pt-3.5">
                <FieldLabel>Signature Proof Attachment Path (URL)</FieldLabel>
                <input
                  type="url"
                ...
                <p className="text-[11px] text-slate-400 mt-1">
                  Attach the captured payload URL verifying official executive sign-off.
                </p>
